import 'package:aplicacao_cliente_hibrida/page/dashboard/layouts/components/widgets/sun_display.dart';
...
class CardLed extends StatefulWidget {
  final String topic;
  final IoTDevice ioTDevice;
  const CardLed({super.key, required this.topic, required this.ioTDevice});

  @override
  State<CardLed> createState() => _CardLedState();
}

class _CardLedState extends State<CardLed> {
          ...
          child: Container(
            ...
            child: Center(
              child: Observer(builder: (_) {
                String title = "";
                if ("Dimerização Vermelha [%]" == widget.ioTDevice.name) {
                  title = AppLocalizations.of(context)!.dimRed;
                }
                if ("Dimerização RBW [%]" == widget.ioTDevice.name) {
                  title = AppLocalizations.of(context)!.dimRBW;
                }
                if ("Dimerização Branca [%]" == widget.ioTDevice.name) {
                  title = AppLocalizations.of(context)!.dimWhite;
                }
                if ("Dimerização Azul [%]" == widget.ioTDevice.name) {
