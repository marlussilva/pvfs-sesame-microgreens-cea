...
The firmware is written in Arduino-style C/C++ and uses non-blocking scheduling (FreeRTOS-style tasks) to ensure continuous acquisition and LED dimming without timing conflicts.

### 2.2 Infrastructure layer (`software/my_api/`, `software/my_mqtt_server/`)

The infrastructure layer is composed of:

- **MongoDB** – document-oriented storage for raw and aggregated time-series data.  
...
- **MQTT broker** – lightweight publish/subscribe communication between ESP32 nodes and the backend.  
  The topic hierarchy separates telemetry, commands and logging.

Both services are usually deployed in Docker containers orchestrated with Docker Compose.

### 2.3 Protocols layer (`software/my_mqtt_server/`, `software/my_api/`)

**DartMQTT Control (`software/my_mqtt_server/`)**  
MQTT client written in Dart responsible for:

- Subscribing to sensor topics from ESP32 nodes;  
- Publishing control commands (e.g., LED dimming profiles);  
- Performing basic validation, session handling and buffering;  
- Aggregating data in short time windows before persisting to the database.

**DartServer Shelf API (`software/my_api/`)**  
REST + WebSocket API implemented in Dart using the Shelf framework. Its main responsibilities are to:
...
- **Regimes**: Constant PPFD; Gaussian-modulated PPFD.  
...
In this repository, data are organized as follows.

### 3.1 `data/` – Energy-use dataset

The `data/` directory contains the dataset used to derive the energy-use metrics reported in the manuscript:
...
This file documents, for each treatment:
...
- Save TXT/CSV summaries, logs and plots in result folders created by the script (e.g., `resultados/`).
...
- Produce text-oriented and tabular outputs, including:
  - ANOVA tables;
  - Treatment means, standard errors and coefficients of variation (CV);
...
Otherwise, install the core scientific stack manually:
